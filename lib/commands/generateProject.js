...
async function installDependencies(projectDir, techStack) {
  ...
  if (["Next.js", "ReactJS", "Node.js + Express"].includes(techStack)) {
...
async function startProject(projectDir, techStack) {
  ...
  // Sleep for 4 seconds
  console.log(chalk.yellow("\n⏳ Waiting for 4 seconds before opening the shell...\n"));
  await new Promise((resolve) => setTimeout(resolve, 4000));

  startShell(techStack);
}
...
async function generateProject() {
  ...
  let response;
  try {
    const aiGenerated = await generateCode(projectDir, techStack);
